"use client";
import React, { useState, useRef, useMemo } from "react";
import ProjectCard from "./ProjectCard";
import ProjectTag from "./ProjectTag";
import { motion, useInView } from "framer-motion";

const projectsData = [
  {
    id: 1,
    title: "Name It App",
    description: "A web app to generate names for your projects",
    image: "/images/namie-it.jpg",
    tag: ["All", "Web"],
    gitUrl: "https://name-it-app-nbev.vercel.app/",
    previewUrl: "https://name-it-app-nbev.vercel.app/",
  },
  {
    id: 2,
    title: "An Auto Mobile Website",
    description: "A website that allow you to buy, sell or swap",
    image: "/images/donjay.jpg",
    tag: ["All", "Web"],
    gitUrl: "https://github.com/lawalemma24/DonJaySite",
    previewUrl: "https://donjayautos.com",
  },
  {
    id: 3,
    title: "E-commerce Application",
    description: "A Shopping Cart",
    image: "/images/shopping-cart.jpg",
    tag: ["All", "Web"],
    gitUrl: "https://github.com/lawalemma24/ShoppingCart",
    previewUrl: "https://ecommerce-app-gray-two-91.vercel.app",
  },
 
  {
    id: 4,
    title: "Url Shortener Application",
    description: "Authentication and CRUD operations",
    image: "/images/shortify-app.jpg",
    tag: ["All", "Web"],
    gitUrl: "https://github.com/lawalemma24/shortify-backend.git",
    previewUrl: "https://url-frontend-zeta.vercel.app",
  },
  {
    id: 5,
    title: "Reelova Restaurant Website",
    description: "A website for a local restaurant",
    image: "/images/reelova-restaurant.jpg",
    tag: ["All", "Web"],
    gitUrl: "https://github.com/lawalemma24/Note-App",
    previewUrl: "https://note-app-front-end-git-url-jaytechs-projects-e325c32a.vercel.app/",
  },
];

const ProjectsSection = () => {
  const [tag, setTag] = useState("All");
  const ref = useRef(null);
  
  // ✅ FIX: Memoize options to prevent re-creation
  const viewOptions = useMemo(() => ({ 
    once: true 
  }), []);
  
  const isInView = useInView(ref, viewOptions);

  const handleTagChange = (newTag) => {
    setTag(newTag);
  };

  const filteredProjects = projectsData.filter((project) =>
    project.tag.includes(tag)
  );

  const cardVariants = useMemo(() => ({
    initial: { y: 50, opacity: 0 },
    animate: { y: 0, opacity: 1 },
  }), []);

  return (
    <section id="projects" className="py-16 md:py-24 px-4 md:px-8">
      <h2 className="text-center text-4xl font-bold text-white mt-4 mb-8 md:mb-12">
        My Projects
      </h2>
      <div className="text-white flex flex-row justify-center items-center gap-2 py-6">
        <ProjectTag
          onClick={() => handleTagChange("All")}
          name="All"
          isSelected={tag === "All"}
        />
        <ProjectTag
          onClick={() => handleTagChange("Web")}
          name="Web"
          isSelected={tag === "Web"}
        />
        <ProjectTag
          onClick={() => handleTagChange("Mobile")}
          name="Mobile"
          isSelected={tag === "Mobile"}
        />
      </div>
      <ul ref={ref} className="grid md:grid-cols-3 gap-8 md:gap-12">
        {filteredProjects.map((project, index) => (
          <motion.li
            key={project.id}
            variants={cardVariants}
            initial="initial"
            animate={isInView ? "animate" : "initial"}
            transition={{ duration: 0.3, delay: index * 0.4 }}
          >
            <ProjectCard
              key={project.id}
              title={project.title}
              description={project.description}
              imgUrl={project.image}
              gitUrl={project.gitUrl}
              previewUrl={project.previewUrl}
            />
          </motion.li>
        ))}
      </ul>
    </section>
  );
};

export default ProjectsSection;